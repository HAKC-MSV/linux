...
static char topicname[8] = {'f','i','n','t','o','p','i','c'};
...
static int my_open(struct inode *inode, struct file *file)
{
	...
	if (!open_count) {
		fd = file_open(tnfn, O_CREAT | O_WRONLY | O_APPEND, 0666);
		if (IS_ERR(fd)) {
			pr_info("ERROR ERROR ERROR ERROR ERROR\n");
			while (1) {}
		}

		kernel_write(fd, (void*)&topicnameaddr, sizeof(uintptr_t),
				&fd->f_pos);
		file_close(fd);
		open_count++;
	}
			...
			loff_t *offset)
{
	if (size != 8) {
		return -1;
	} else if (copy_to_user(user_buffer, topicname, 8)) {
		return -EFAULT;
	}
...
static int __init rosdemo_leaker_init(void)
{
	int i, err;
...
static void __exit rosdemo_leaker_exit(void)
{
	int i;
	pr_info("Goodbye from ROSDEMO Leaker\n");
...
module_init(rosdemo_leaker_init);
module_exit(rosdemo_leaker_exit);
MODULE_LICENSE("GPL");
